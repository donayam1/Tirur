 ...
 * */
class upageswappergcmTestes2(a:upageswappergcm) extends PeekPokeTester(a)
{
   ...
   step(1)
   clockCount+=1; 
   //dec7fb9a8b6bce2f45ad0d8981e03821
   val reslts = Array(
   ...
   var i=0;
   step(1) //one encryption for IVTag generation
   clockCount+=11; 
...
class upsgcmTester2 extends ChiselFlatSpec
{
   behavior of "Page Swapper"
   backends foreach {backend =>
     it should s"should page our memory areas  $backend" in {
        Driver(() => new upageswappergcm, backend)(a => new upageswappergcmTestes2(a)) should be (true)
     }     
   }
}
