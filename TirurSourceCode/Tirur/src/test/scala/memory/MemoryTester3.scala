...
class MemoryTestes3(m:Memory3) extends PeekPokeTester(m)
{ 
  for( i <- 0 to 40)
  {
...
class MemoryTester3 extends ChiselFlatSpec{
  behavior of "Memory"
  backends foreach{ backend =>
    it should s"read and write to some address $backend" in{
      Driver(() => new Memory3,backend)(m => new MemoryTestes3(m))should be(true)
    }
