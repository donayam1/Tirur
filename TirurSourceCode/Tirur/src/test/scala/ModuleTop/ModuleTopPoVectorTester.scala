 ...
 * */
class ModuleTopPoVectorTestes(a:ModuleTop) extends PeekPokeTester(a)
{
   ...
   val reslts = Array(
      "h4ee723e0aba50c0ec4d881850823dc95".U,
...
class ModuleTopPoVectorTester extends ChiselFlatSpec
{
   behavior of "Page Swapper"
   backends foreach {backend =>
     it should s"should page our memory areas  $backend" in {
        Driver(() => new ModuleTop, backend)(a => new ModuleTopPoVectorTestes(a)) should be (true)
     }     
   }
}
