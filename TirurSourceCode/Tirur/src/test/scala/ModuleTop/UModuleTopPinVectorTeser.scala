 ...
 * */
class UModuleTopPinVectorTestes(a:UModuleTop) extends PeekPokeTester(a)
{
   ...
   while(peek(a.io.done) == BigInt(0))
   {
     ...
     if(i==0)
     {
       ...
       poke(a.io.io.wdata,9.U)   //page in memory
       poke(a.io.io.valid,true.B)
       poke(a.io.io.we,true.B)
       poke(a.io.io.en,true.B)
       step(1)
   ...
   expect(a.io.err,true.B) //final tag shoud be 546b04f9ce679bfebe058b3ec59f106f
   expect(a.io.currOutReady,false.B)
                 
         
}

class UModuleTopPinVectorTester extends ChiselFlatSpec
{
   behavior of "Page Swapper"
   backends foreach {backend =>
     it should s"should page our memory areas  $backend" in {
        Driver(() => new UModuleTop, backend)(a => new UModuleTopPinVectorTestes(a)) should be (true)
     }     
   }
}
