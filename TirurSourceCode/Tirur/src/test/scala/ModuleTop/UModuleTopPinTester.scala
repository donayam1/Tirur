 ...
 * */
class UModuleTopPinTestes(a:UModuleTop) extends PeekPokeTester(a)
{  
   var clockCount=0;
...
class UModuleTopPinTester extends ChiselFlatSpec
{
   behavior of "Page Swapper"
   backends foreach {backend =>
     it should s"should page our memory areas  $backend" in {
        Driver(() => new UModuleTop, backend)(a => new UModuleTopPinTestes(a)) should be (true)
     }     
   }
}
