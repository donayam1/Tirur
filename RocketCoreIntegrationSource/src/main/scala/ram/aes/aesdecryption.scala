...
class aes_dec_io extends Bundle{
...
class dec extends Module{
  val io =IO(new aes_dec_io)
  
  //io <> DontCare 
  
  val rCounter = RegInit(15.U(4.W))
  val nstate   = RegInit(0.U(128.W))//0.U(128.W)
  io.done := Mux(io.en & io.rkeyDone,rCounter === 10.U,false.B)
  ...
  rCounter := Mux((rCounter>=10.U && rCounter<15.U)|(~io.en),15.U, rCounter + 1.U)
  //rCounter := Mux(io.en, rCounter + 1.U,0.U)
  //when(io.en)
  //{
  io.roundOut := Mux(io.en,10.U - rCounter,0.U) //Mux(io.rkeyDone,10.U - rCounter,rCounter)
  //}
  
  
  val st1 = addRoundKey(io.rkey,io.data)
  val st2 = shiftRow(st1)
  val w   = byteSubs(st2)
           
  
  
    val st12= addRoundKey(io.rkey,nstate)
    val st112 = mixColums(st12)
    val st22= shiftRow(st112)
    val ans= byteSubs(st22)
  
  
  
  
  val output = MuxLookup(rCounter,
              ans,
              Array(
                0.U -> w,
                10.U -> st12
  ...
  when(io.en)
  { 
    //printf("aes dec - enable = %x\n",io.en)  
    //printf(p"round out= ${io.roundOut} and round=${rCounter}\n")
    when(io.rkeyDone)
    {
      
      //printf("decryption key done \n")
      when(rCounter  === 0.U)
      {     
        //printf("round 0 data=%x\n",io.data)
        //val st1= addRoundKey(io.rkey,io.data)
        //val st2= shiftRow(st1)
        //val w= byteSubs(st2)
        nstate := w            
      ...
      }.elsewhen((rCounter>0.U)&(rCounter <= 9.U))
      {
        //printf(p"other round out= ${io.roundOut} and rcounter=${rCounter}\n")
//        val st1= addRoundKey(io.rkey,nstate)
//        val st11 = mixColums(st1)
//        val st2= shiftRow(st11)
//        val ans= byteSubs(st2)
        nstate := ans
  
//        io.out := ans
        
      }.elsewhen(rCounter === 10.U)
      {
//        val st1= addRoundKey(io.rkey,nstate)
//        io.out := st1
        nstate := st12//st1
...
//class decRKG_io extends Bundle{
//  val data = Input(UInt(128.W))
//  val done = Output(Bool())
...
//
//class decRKG extends Module{
//  val io= IO(new decRKG_io)
//  
//  val decrm = Module(new dec)
//  val rkgm = Module(new expander)
//  val sboxm =Module(new sbox)
//  
//  sboxm.io <> DontCare 
//  rkgm.io  <> DontCare
//  sboxm.io <> DontCare 
//  
//  decrm.io.rkey := rkgm.io.data 
//  decrm.io.rkeyDone := rkgm.io.done 
//  //printf(p"key g done= 0x${Hexadecimal(rkgm.io.done )}\n")
//  
//  decrm.io.data := io.data
//   
//  rkgm.io.round := decrm.io.roundOut
//  //printf(p"round= 0x${Hexadecimal(decrm.io.roundOut)}\n")
//  rkgm.io.dir := false.B
//  rkgm.io.key := io.key 
//  
//  
//  sboxm.io.addr := rkgm.io.sboxio.addr
//  rkgm.io.sboxio.data := sboxm.io.data
//  
//  io.done := decrm.io.done 
//  io.out := decrm.io.out 
//  
//  
//}
...
object elaborateaesdec extends App{
  chisel3.Driver.execute(args,()=> new dec)
}
