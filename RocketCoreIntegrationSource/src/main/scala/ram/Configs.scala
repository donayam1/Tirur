...
class WithSRAM extends Config((site, here, up) => {
  case BuildTop => (p: Parameters) =>
    Module(LazyModule(new ExampleTopWithSRAM()(p)).module)
})*/
...
//class SRAMConfig extends Config(new WithSRAM ++ new BaseExampleConfig)

class SRAMConfig extends Config(new WithSRAM ++ new DefaultExampleConfig)

class SRAMJtagConfig extends Config(new SRAMConfig ++ new WithJtagDTM)
